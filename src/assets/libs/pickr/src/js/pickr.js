...
class Pickr {
      ...
      palette: Moveable({
        ...
        onchange(x, y) {
          ...
          this.wrapper.style.background = `
                        ...
                        linear-gradient(to left, hsla(${_color.h}, 100%, 50%, ${
            _color.a
          }), rgba(255, 255, 255, ${_color.a}))
                    `
  ...
  _bindEvents() {
    ...
    const eventBindings = [
      ...
      _.on(_root.preview.lastColor, 'click', () =>
        this.setHSVA(...this._lastColor.toHSVA()),
      ),
    ...
    if (!options.showAlways) {
      ...
      eventBindings.push(
        ...
        _.on(
          ...
          'keyup',
          (e) =>
            this.isOpen() && (e.key === ck || e.code === ck) && this.hide(),
        ),
        ...
        _.on(
          ...
          (e) => {
            if (
              this.isOpen() &&
              !_.eventPath(e).some(
                (el) => el === _root.app || el === _root.button,
              )
            ) {
              this.hide()
            }
  ...
  _updateOutput() {
    ...
    if (this._root.interaction.type()) {
      ...
      const method = `to${this._root.interaction
        .type()
        .getAttribute('data-type')}`
      this._root.interaction.result.value =
        typeof this._color[method] === 'function'
          ? this._color[method]().toString()
          : ''
    }
  ...
  setColor(string, silent = false) {
    ...
    if (values) {
      ...
      const target = options.find((el) =>
        el.getAttribute('data-type').startsWith(utype),
      )
  ...
  setColorRepresentation(type) {
    ...
    return !!this._root.interaction.options.find(
      (v) => v.getAttribute('data-type').startsWith(type) && !v.click(),
    )
  }
